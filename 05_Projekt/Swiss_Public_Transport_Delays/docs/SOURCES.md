...
The final analysis should use the daily files collected by the group. They contain scheduled times and actual values or the last available forecast. According to the publisher, records without real-time information can be absent and the data have limited suitability for exact punctuality measurement. Record the exact resource URLs and collection dates in the final report.
